...
type Panel = "none" | "magazine" | "settings" | "search";
      ...
      <div ref={shellRef} className="relative mx-auto w-full max-w-[1400px]">
        <div className="flex items-center gap-3 rounded-clay-lg bg-clay-surface/95 p-2.5 shadow-clay backdrop-blur-xl sm:gap-4 sm:p-3">
          ...
          <div className="ml-auto flex items-center gap-2 sm:gap-2.5">
            ...
            <NavIconButton
              label="Magazine"
              active={panel === "magazine"}
              onClick={() => openPanel("magazine")}
            >
        ...
        {/* magazine popover — "Your Travel Guide" */}
        <AnimatePresence>
          {panel === "magazine" && (
            <Popover>
              <div className="mb-3 border-b border-clay-muted/15 pb-2.5">
                <h3 className="font-display text-lg font-bold text-clay-ink">Your Travel Guide</h3>
                <p className="font-body text-xs text-clay-ink-soft mt-0.5">
                  Interactive 3D digital magazines & place guides
                </p>
              </div>

              <div className="space-y-2 max-h-[360px] overflow-y-auto pr-1">
                {Object.values(FLIPBOOK_DATA).map((dest) => (
                  <Link
                    key={dest.id}
                    ...
                    className="flex items-center gap-3 p-2.5 rounded-clay-sm bg-clay-sunken/60 hover:bg-clay-sunken shadow-clay-inset-sm transition-all group"
                  >
                    <div className="w-10 h-10 rounded-xl overflow-hidden shrink-0 relative shadow-clay-xs">
                      <img
                        src={dest.coverImage}
                        alt={dest.name}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between">
                        <span className="font-display text-sm font-bold text-clay-ink group-hover:text-clay-tangerine transition-colors">
                          {dest.name}
                        </span>
                        <span className="font-body text-[10px] font-extrabold uppercase text-clay-muted">
                          {dest.region}
                        </span>
                      </div>
                      <p className="font-body text-[11px] text-clay-ink-soft truncate mt-0.5">
                        {dest.subtitle}
                      </p>
                    </div>
                    <ChevronRightIcon size={16} className="text-clay-muted group-hover:text-clay-ink transition-colors" />
                  </Link>
                ))}
              </div>

              <div className="mt-3 pt-2 border-t border-clay-muted/15 text-center">
                <Link
                  href="/destinations"
                  onClick={() => {
                    play("nav");
                  ...
                  className="font-body text-xs font-bold text-clay-ink hover:underline inline-flex items-center gap-1"
                >
                  Show All Destinations <ChevronRightIcon size={14} />
                </Link>
              </div>
            </Popover>
          )}
